      ...
      .filter((g) => {
        return (
          (!filters.submitter_name ||
            g.submitter_name
              .toLowerCase()
              .includes(filters.submitter_name.toLowerCase())) &&
          (!filters.unit ||
            g.unit.toLowerCase().includes(filters.unit.toLowerCase())) &&
          (!filters.type ||
            g.grievance_type
              ?.toLowerCase()
                ...
                {:else if column.field === "status"}
                  <div class="status-container">
                    {#if grievance.status}
                      ...
                      <StatusIcon
                        size={16}
                        class="status-icon {grievance.status}"
                      />
                    {/if}
                    {#if canEditStatus && !readonly}
                      <select
                        ...
                        onchange={(e) =>
                          handleStatusChange(grievance, e.currentTarget.value)}
                        class="status-select"
                        class:status-pending={grievance.status === "pending"}
                        class:status-in-progress={grievance.status ===
                          "in_progress"}
